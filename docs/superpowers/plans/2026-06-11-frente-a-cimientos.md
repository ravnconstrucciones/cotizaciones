...
> **AJUSTE AUTORITATIVO (verificación prod 2026-06-12):** Las 4 tablas `inmobiliario_*` NO EXISTEN en producción (la migración `20260522120000_inmobiliario_schema.sql` está en el repo pero nunca fue aplicada). Todo el SQL de inmobiliario va envuelto en guards `DO $$ BEGIN IF to_regclass(...) IS NOT NULL THEN ... END IF; END $$;` para que la migración no falle si las tablas no existen. Si `20260522120000` se aplica primero o después, el RLS de este archivo llegará igualmente cuando `db push` ejecute esta migración.
