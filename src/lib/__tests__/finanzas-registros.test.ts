...
import {resumirGastosPersonales,semanaRegistro,csvRegistros,categoriaPersonal,type GastoPersonalRegistro} from "../finanzas-registros";
...
describe("registros financieros",()=>{
  it("agrupa variantes históricas de categorías sin perder gastos ni cambiar originales",()=>{const filas=["Súper","Supermercado"," SUPERMERCADO ","Ocio","ocio"].map((categoria,i)=>({...g,id:String(i),categoria}));const r=resumirGastosPersonales(filas);expect(r.total).toBe(501.25);expect(r.categorias.map(c=>[c.nombre,c.total])).toEqual([["Supermercado",300.75],["Ocio",200.5]]);expect(filas.filter(g=>categoriaPersonal(g.categoria)==="Supermercado")).toHaveLength(3);expect(filas[0].categoria).toBe("Súper");});
