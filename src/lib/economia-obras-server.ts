...
export async function cargarEconomiaObras(presupuestoId?: string) {
  ...
  const [pres, obras, gastos, acuerdos, items, movimientosGlobales] = await Promise.all([
    ...
    readAll<GastoEconomico>(sb, "presupuestos_gastos", "id,presupuesto_id,fecha,descripcion,importe,rubro_id,mo_acuerdo_id,operario_id,plan_item_id,cashflow_item_id,cuenta_id", filtro),
