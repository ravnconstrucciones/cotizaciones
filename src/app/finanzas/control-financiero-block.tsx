...
export function ControlFinancieroBlock({mode = "economia"}: {mode?: "economia" | "compras" | "resumen"}) {
  ...
  return <motion.section initial={reduced?false:{opacity:0}} animate={{opacity:1}} transition={{duration:.2}} aria-label={mode==="compras"?"Plan de compras":"Control financiero"} className="mb-6 border border-cdm-line bg-cdm-panel p-5 sm:p-6">
    ...
    {data&&estado==="ok"&&<>
      ...
      {mode!=="compras"&&<>
        <dl className="mt-6 grid gap-6 md:grid-cols-3">{indicadores?.map(i=><div key={i.nombre} className="border-t border-cdm-line pt-3"><dt className="text-sm">{i.nombre}</dt><dd className="mt-3 break-words text-2xl font-semibold tabular-nums tracking-tight">{i.valor}</dd><dd className="mt-3 text-sm leading-relaxed text-cdm-muted">{i.detalle}</dd></div>)}</dl>
        {mode==="economia"&&<>
          ...
          <details className="border-t border-cdm-line"><summary className="min-h-12 cursor-pointer py-4 font-semibold">Cuánto dejan los proyectos</summary><p className="mb-4 text-sm text-cdm-muted">Margen bruto: falta descontar estructura e impuestos. Una obra abierta tiene resultado provisional. La diferencia bruta no indica cuánto cobraste ni cuánto tenés disponible.</p><ul className="grid gap-5 sm:grid-cols-2">{data.margenes.map(m=><li key={m.obra} className="border border-cdm-line p-4"><h3 className="font-semibold">{m.obra}</h3><p className="mt-2 text-2xl font-semibold">{(m.margen*100).toFixed(1)}%</p><p className="mt-2 text-sm">Venta {formatMoneyInt(m.venta)} · costo {formatMoneyInt(m.costo)}</p><p className="mt-1 text-sm">Diferencia bruta {formatMoneyInt(m.venta-m.costo)}</p><p className="mt-3 text-sm leading-relaxed text-cdm-muted">{m.nota}</p></li>)}</ul></details>
        </>}
      </>}
      {mode==="compras"&&<>
