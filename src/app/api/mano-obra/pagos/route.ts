...
import {nombreOperario,type Operario} from "@/lib/operarios";
export async function GET(){try{
  const sb=createSupabaseAdminClient();const [acuerdos,gastos,obras,cuentas,operarios]=await Promise.all([
    readAll<AcuerdoMO>(sb,"mo_acuerdos","id,presupuesto_id,persona,trabajo,monto_arreglado,moneda,estado,notas,created_at"),
    readAll<GastoEconomico & PagoMO>(sb,"presupuestos_gastos","id,presupuesto_id,mo_acuerdo_id,operario_id,importe,fecha,descripcion,cotizacion_venta_ars_por_usd,cuenta_id"),
    ...
    readAll<Operario>(sb,"mo_operarios","id,nombre,aliases"),
  ]);
  return NextResponse.json({pagos:registroPagos(acuerdos,gastos,obras,cuentas,operarios),acuerdos:resumirAcuerdos(acuerdos,gastos).map(r=>({id:r.acuerdo.id,presupuestoId:r.acuerdo.presupuesto_id,persona:nombreOperario(r.acuerdo.persona,operarios),trabajo:r.acuerdo.trabajo,notas:r.acuerdo.notas,moneda:r.acuerdo.moneda,arreglado:Number(r.acuerdo.monto_arreglado),pagado:r.pagado,saldo:r.saldo,estado:r.acuerdo.estado,pagosSinCotizacion:r.pagosSinCotizacion})),actualizadoAt:new Date().toISOString()},{headers:{"Cache-Control":"private, no-store"}});
