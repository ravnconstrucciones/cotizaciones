...
const state=vi.hoisted(()=>({writes:[] as {table:string;payload:Record<string,unknown>}[],acuerdoPres:"p",acuerdoPersona:"Fran",operarioValido:true,fijoOwner:"personal",dedupeError:false}));
vi.mock("@/lib/supabase/server",()=>({createSupabaseAdminClient:()=>({from:(table:string)=>{
  ...
  const resolve=()=>{if(state.dedupeError&&table==="gastos_personales"&&!write)return{data:null,error:{message:"Network"}};const data=write?{id:`new-${table}`}:{cuentas:{id:"c",moneda:"ARS"},presupuestos:{id:"p",presupuesto_aprobado:true},obras:{id:"o"},mo_acuerdos:{id:"a",presupuesto_id:state.acuerdoPres,persona:state.acuerdoPersona},mo_operarios:state.operarioValido?{id:"f",nombre:"Francisco Pacheco (Fran)",aliases:["Fran"]}:null,obra_plan_items:{id:"plan",presupuesto_id:"p"},finanzas_fijos:{id:"f",dueno:state.fijoOwner}}[table];return {data:data??null,error:null};};
...
beforeEach(()=>{state.writes=[];state.acuerdoPres="p";state.acuerdoPersona="Fran";state.operarioValido=true;state.fijoOwner="personal";state.dedupeError=false;});
describe("registro desde relevos",()=>{
  ...
  it("conserva el destinatario de un pago sin inventar un acuerdo",async()=>{const r=await post({tipo:"obra",presupuesto_id:"p",importe:18,operario_id:"f",descripcion:"Electricidad"});expect(r.status).toBe(200);expect(state.writes.find(w=>w.table==="presupuestos_gastos")?.payload).toMatchObject({importe:18,operario_id:"f",mo_acuerdo_id:null});});
  it("rechaza un destinatario inexistente antes de mover dinero",async()=>{state.operarioValido=false;expect((await post({tipo:"obra",presupuesto_id:"p",importe:18,operario_id:"f"})).status).toBe(400);expect(state.writes).toHaveLength(0);});
  it("rechaza un pago con operario y acuerdo incompatibles",async()=>{state.acuerdoPersona="Carlos";expect((await post({tipo:"obra",presupuesto_id:"p",importe:18,operario_id:"f",mo_acuerdo_id:"a"})).status).toBe(400);expect(state.writes).toHaveLength(0);});
});
